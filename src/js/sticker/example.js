...
// 绘制函数；
    ...
    function showDialog(type, ops) {
        ...
        switch (type) {
            case 'image':
                html = '<li>Choose sticker: <select id="sticker_select"><option value ="ear">ear</option><option value ="nose">nose</option><option value ="neck">neck</option></select>' + '</li>\n                    <li>options:{</li>\n                    <li>' + tab + 'width:<input data-type=\'width\' id="width" class=\'js-input input\' type=\'text\' value=\'' + ops.options.width + '\'></li>\n                    <li>' + tab + 'pos:{</li>\n                    <li>' + (tab + tab) + 'x:<input data-type=\'x\' id="x" class=\'js-input input\' type=\'text\' value=\'' + ops.options.pos.x + '\'></li>\n                    <li>' + (tab + tab) + 'y:<input data-type=\'y\' id="y" class=\'js-input input\' type=\'text\' value=\'' + ops.options.pos.y + '\'></li>\n                    <li>' + (tab + tab) + 'scale:<input data-type=\'scale\' class=\'js-input input\' type=\'text\' value=\'' + ops.options.pos.scale + '\'></li>\n                    <li>' + (tab + tab) + 'rotate:<input data-type=\'rotate\' class=\'js-input input\' type=\'text\' value=\'' + ops.options.pos.rotate + '\'></li>\n                    <li>' + tab + '}</li>\n                    <li>}</li>';
    ...
    $(window).on('change', '#sticker_select', function () {
        var $this = $(this);
        var sticker_type = $this.val();
        var ops = $sure.data('ops');
        switch (sticker_type) {
            case 'ear':
                ops.image = './img/sticker/ear.png';
                ops.options.width = 440;
                ops.options.pos.x = 90;
                ops.options.pos.y = 35;
                $('#width').val(440);
                $('#x').val(90);
                $('#y').val(35);
                break;
            case 'nose':
                ops.image = './img/sticker/nose.png';
                ops.options.width = 120;
                ops.options.pos.x = 225;
                ops.options.pos.y = 320;
                $('#width').val(120);
                $('#x').val(225);
                $('#y').val(320);
                break;
            case 'neck':
                ops.image = './img/sticker/neck.png';
                ops.options.width = 300;
                ops.options.pos.x = 170;
                ops.options.pos.y = 520;
                $('#width').val(300);
                $('#x').val(170);
                $('#y').val(520);
                break;
